...
const shouldFail = async (connection: Connection, programId: PublicKey, feePayer: Keypair, acct: Keypair, message: string) => {
  ...
  try {
    ...
    console.log("second failed: ", error)
  }
  return txid
}
...
let txid = undefined
  ...
  setTimeout(async () => {
    await shouldFail(connection, programId, feePayer, echoBuffer, "should-fail")
  }, 4000)
};
